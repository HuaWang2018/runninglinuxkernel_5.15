...
- 网络桥接是在数据链路层（OSI第二次）工作的，它能够将多个网络接口链接在一起，使它们看起来仿佛在同一个物理网络中。
- 其作业是使所有加入的接口处于同一广播域内，可以直接进行通信，就像连接在同一交换机上一样。
- 在家用网络来说，就是可以和你的PC一样，可以从你家的路由器上通过DHCP获取地址。
